...
from app.models import TelegramUser, OriginalString, YandexTranslate, TatsoftTranslate, GoogleTranslate
...
def add_yandex(original, translate, length):
    try:
        r = YandexTranslate(original=original, translate=translate, length=length, yandex_score=0)
        r.save()
        return r
    except Exception as e:
        print(e)
...
def add_tatsoft(original, translate, length):
    try:
        r = TatsoftTranslate(original=original, translate=translate, length=length, tatsoft_score=0)
        r.save()
        return r
    except Exception as e:
        print(e)
...
def add_google(original, translate, length):
    try:
        r = GoogleTranslate(original=original, translate=translate, length=length, google_score=0)
        r.save()
        return r
    except Exception as e:
        print(e)
...
def update_yandex_score(translate_id, score):
    return YandexTranslate.objects.filter(id=translate_id).update(yandex_score=score)


@sync_to_async
def update_tatsoft_score(translate_id, score):
    return TatsoftTranslate.objects.filter(id=translate_id).update(tatsoft_score=score)


@sync_to_async
def update_original_score(original: OriginalString):
    orig = OriginalString.objects.filter(pk=original.pk).first()
    yandex = orig.get_yandex
    google = orig.get_google
    tatsoft = orig.get_tatsoft
    rate = tatsoft.tatsoft_score + google.google_score + yandex.yandex_score
    print(rate)
    OriginalString.objects.filter(pk=orig.pk).update(rates=rate)
